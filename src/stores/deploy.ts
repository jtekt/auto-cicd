...
export const useDeployStore = defineStore("deploy", () => {
  ...
  const originalEnvironmentVariables = ref<string | null>(null);
  ...
  const isOutputFileInvalid = computed(() => {
    // If user cannot change the file output (Auto generated by the build then do not display it)
    if (!currentProjectFrameworkConfig.value?.userConfigurable?.outputFile)
      return false;
  ...
  const envChanges = computed(() => {
    ...
    const originalEnv = originalEnvironmentVariables.value
      ? parseEnv(originalEnvironmentVariables.value)
      : [];
  ...
  async function openDeployment(selectedProject: ProjectNode) {
    ...
    originalEnvironmentVariables.value = envs;
  ...
  async function confirmDeploy() {
    ...
    const actions = {
      ...
      env:
        (originalEnvironmentVariables.value &&
